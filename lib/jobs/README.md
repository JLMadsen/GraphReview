...
| `github-access.ts` | Joins a stored `Repo` to `lib/github`: parses owner/repo out of the URL, decrypts the PAT from `Settings` (§11), and resolves `{owner, repo, token}` access for a repo. |
