...
From the project's background on background jobs:
...
| `source.ts` | Resolving a `Repo` to a directory: the `LOCAL_REPOS_PATH` containment check (a security boundary) and the app-managed clone/fetch into `REPO_CACHE_DIR`. Also the cheap HEAD probes (`git ls-remote` / `rev-parse`). |
...
| `module-tier.ts` | `writeModuleTier()` — the one writer of the module tier (DESIGN.md §6.3): ownership → folder/merged module nodes → `BELONGS_TO` (only changed files) → `DEPENDS_ON` → prune empty folder modules → domain inheritance → findings follow their files → merge suggestions. Called by `analyze.ts` and by `regroupRepo()` (after accept/unmerge, from the stored graph — no parsing). |
...
| `merges.ts` | Accept/reject/reopen a suggestion, unmerge/rename a merged module. Not re-exported (pulls in the folder clustering). |
