// The "not linked to GitHub" / "no PAT configured" empty states shared by
// the Branches and Pull Requests tabs (§4, decision #7).
//
// These are deliberately not errors: a `local` repo without a GitHub URL is
// a perfectly valid, fully analyzable repo — it just has no branches or PRs
// to list from the API. The visual treatment says so: the informational
...
import type { GitHubUnavailableReason } from "@/lib/jobs";
...
export function GitHubNotice({
  ...
  if (reason === "invalid_url") {
    return (
      <NoticeCard icon={Link2Off} title="Unrecognized GitHub URL">
        The URL stored for this repo isn&apos;t a{" "}
        <span className="font-mono text-foreground/80">github.com/owner/repo</span>{" "}
  ...
  return (
    <NoticeCard icon={HardDrive} title="Not linked to GitHub">
      This repo was added as a local source, so there are no {subject} to
